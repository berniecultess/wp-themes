	...
	if( is_single() ) {
		switch ( $format ) {
			...
			case "video":
				...
				$output .= '</div>';
				// $output .='<a href="'.get_permalink(get_the_ID()).'" class="blog-read-more" >'.__('Continue Reading','be-themes').'</a>';
				$output .= '</div>';
				break;
			case "audio":
				$output .= '<div class="'.implode(' ',get_post_class('blog-post')).'">';
			...
			case "gallery":
				...
				break;
			case "quote":
				$output .= '<div class="'.implode(' ',get_post_class('blog-post')).' quote"><div class="post-item-wrap sec-bg">';
				...
				break;
			case "link":
				$output .= '<div class="'.implode(' ',get_post_class('blog-post')).'"><div class="post-item-wrap sec-bg">';
				$output .= '<div class="blog-post-content-area-wrap clearfix">';
				$output .= '<div class="blog-post-content-area">';
			...
			default :
				...
				$output .= '<a href="'.get_permalink(get_the_ID()).'" class="recent-post-comment-count">'.get_comments_number().' '.__('Comments','be-themes').'</a>';
				$output .= '<span class="blog-detail-sep"> | </span>';				
				$output .= get_the_category_list(', '); //get_be_themes_category_list(get_the_ID());
				$output .= '</div>';
				$output .= '<hr class="separator style-1 blog-title-sep" />';
				$output .= '<div class="blog-post-content sec-color">';
				$output .= apply_filters('the_content',get_the_content());
		...
		switch ($format) {
			...
			case "quote":
				...
				$output .= '<h6 class="blog-title text-align-center"><a href="'.get_permalink(get_the_ID()).'">"'.get_post_meta(get_the_ID(),'be_themes_quote_title',true).'"</a></h6>';
				$output .= '<p class="quote-author text-align-center"><a href="'.get_post_meta(get_the_ID(),'be_themes_quote_author_link',true).'">'.get_post_meta(get_the_ID(),'be_themes_quote_author',true).'</a></p>';
				$output .= '<p class="post-nav text-align-center">';
				$output .= '<span class="recent-post-time">'.get_the_date( "F j, Y" ).'</span>';
				$output .= '<span class="blog-detail-sep"> | </span>';
				$output .= '<a href="'.get_permalink(get_the_ID()).'" class="recent-post-comment-count">'.get_comments_number().' '.__('Comments','be-themes').'</a>';
				$output .= '</p>';
			...
			case "link":
				...
				$output .= '<h6 class="blog-title text-align-center"><a href="'.get_post_meta(get_the_ID(),'be_themes_link_format',true).'" target="_blank">'.get_the_title().'</a></h6>';
				$output .= '<p class="post-nav text-align-center">';
				$output .= '<span class="recent-post-time">'.get_the_date( "F j, Y" ).'</span>';
				$output .= '</p>';
				$output .= '</div>';
